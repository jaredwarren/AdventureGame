...
func TestEnemyChaseTarget_NextTile(t *testing.T) {
	...
	tgx, tgy := w.EnemyChaseTarget(
		float64(0)*TileSize+TileSize*0.5,
		float64(2)*TileSize+TileSize*0.5,
		float64(4)*TileSize+TileSize*0.5,
		float64(2)*TileSize+TileSize*0.5,
	)
	wantX := float64(1)*TileSize + TileSize*0.5
	wantY := float64(2)*TileSize + TileSize*0.5
